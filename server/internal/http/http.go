...
func GetWeeklyStats(dataSource *sql.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		weeklyStats, err := db.GetWeeklyStats(dataSource, userID)
		if err != nil {
			c.JSON(http.StatusInternalServerError, fmt.Sprintf("Error reading weekly stats: %q", err))
			return
		}
